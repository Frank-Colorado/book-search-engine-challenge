...
const resolvers = {
  ...
  Mutation: {
    ...
    // This is a mutation called removeBook that returns a User type.
    // The bookId is destructured from the args argument.
    removeBook: async (_root, { bookId }, context) => {
      // If the context contains a user
      if (context.user) {
        // Find the user with the id that was stored in the token
        // And remove the book from the user's savedBooks array
